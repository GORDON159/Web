...
<html lang="en">
        ...
        <nav class="navbar navbar-expand-lg navbar-dark bg-primary fixed-top" id="sideNav">
            ...
            <div class="collapse navbar-collapse" id="navbarResponsive">
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item"><a class="nav-link js-scroll-trigger" href="#person">更改會員資料</a><li>
					<li class="nav-item"><a class="nav-link js-scroll-trigger" href="login.php">登出</a><li>
                </ul>
            </div>
        </nav>
        <!-- Page Content-->
            ...
            <section class="resume-section" id="experience">
                <div class="resume-section-content">
					...
					<form action="schedule.php" method="post">
						...
						<input type="text" name="time" placeholder="請輸入次數/時間" size="35" maxlength="35">
					...
					function php_func(){
						require('config.php');
						$account = $_COOKIE['ac'];
						$conn=mysqli_connect($db_host,$db_user,$db_pass,$db_name,33060);
                    ...
                    <select name="set" style="width:172px">
		            <option value="">請選擇推薦運動套餐	
       	            </option><option value="全身">全身
		            </option><option value="下半身">下半身
		            </option><option value="手臂">手臂
		            </option><option value="胸部">胸部
		            </option><option value="腹部">腹部
		            </option><option value="腿部">腿部
		            </option><option value="肩膀以及背部">肩膀以及背部
                    </option></select>
                    <input type="submit" value="確定" onclick="set_func()"><br>
                    <?php
                        $train = $_POST["set"];
						require('config.php');
						$conn=mysqli_connect($db_host,$db_user,$db_pass,$db_name,33060);
                        $sql = "SELECT * FROM train"; //查詢語句--查詢資料庫表
						$result = mysqli_query($conn, $sql);
                        if (mysqli_num_rows($result) > 0) {
							while ($row = mysqli_fetch_assoc($result)) {
								echo $row["$train"],"<br>";
    ...
    <div class="container">  
	  ...
	  <form action="upload.php" method="post" enctype="multipart/form-data">
        <input name="fileToUpload" id="fileToUpload" type="file" accept="video/*,image/*">
		<input type="submit" value="確認送出" name="submit">
        <label for="fileToUpload" class="button-primary"></label>
	  </form>
      </section>
...
6.油脂類:一湯匙油等於一片乳酪
        ...
        <script src="js/scripts.js"></script>
            ...
            <hr class="m-0" />
            <!-- Interests-->
            <section class="resume-section" id="person">
                <div class="resume-section-content">
                    <h2 class="mb-5">更改會員資料</h2>
                    <form action="chname.php" method="post">
						更改名字:<input type="text" name="pname" size="35" maxlength="35">
                        <input type="submit" value="確認"/>	
                    </form>
                    <form action="chheight.php" method="post">
						更改身高:<input type="text" name="pheight" size="35" maxlength="35">
                        <input type="submit" value="確認"/>	
                    </form>
                    <form action="chweight.php" method="post">
                        更改體重:<input type="text" name="pweight" size="35" maxlength="35">
                        <input type="submit" value="確認"/>				
					</form>	
                    <form action="chphone.php" method="post">
                        更改電話:<input type="text" name="pphone" size="35" maxlength="35">
                        <input type="submit" value="確認"/>				
					</form>
                    <br>
                    <p>你的帳號:<?php $account = $_COOKIE['ac'];echo $account;?> <br></p>
                    <p>你的名字:
                    <?php 
                    ...
                    $select=mysqli_query($link,"SELECT * FROM `susers` WHERE email='$account'");
                    while ($row = mysqli_fetch_array($select, MYSQLI_BOTH)) {
                        printf($row["name"]);
                    }
                    ?></p><br>
                    <p>你的身高:
                    <?php $select=mysqli_query($link,"SELECT * FROM `susers` WHERE email='$account'");
                    while ($row = mysqli_fetch_array($select, MYSQLI_BOTH)) {
                        printf($row["height"]);
                    }?>
                    </p><br>
